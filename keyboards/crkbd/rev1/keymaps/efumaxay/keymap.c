...
#define MY_CTL MT(MOD_RCTL, KC_NO)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_BASE] = LAYOUT_split_3x6_3(
        ...
        QK_REP,   KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,                         KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,  KC_EQL,
    ...
    [_NUM] = LAYOUT_split_3x6_3(
        ...
        KC_LSFT,  KC_F11,   KC_F12,   _______,  _______,  VIM_G,                        _______,  _______,  _______,  _______,  _______,  _______,
    ...
    [_ADJUST] = LAYOUT_split_3x6_3(
        ...
        _______,  _______,  _______,  _______,  _______,  _______,                      _______,  CTL_E,    CTL_Y,    _______,  _______,  _______,
                                                _______,  _______,  _______,  _______,  _______,  _______
    ),
};
...
bool combo_should_trigger(uint16_t combo_index, combo_t *combo, uint16_t keycode, keyrecord_t *record) {
    switch (combo_index) {
        case COMBO_HRC_NUM_L_CTL:
        case COMBO_HRC_NUM_L_SFT:
        case COMBO_HRC_NUM_R_CTL:
        case COMBO_HRC_NUM_R_SFT:
        case COMBO_ESC_NUM_L:
        case COMBO_ESC_NUM_R:
            return get_highest_layer(layer_state) == _NUM;

        case COMBO_ESC_SYM_L:
        case COMBO_ESC_SYM_R:
            return get_highest_layer(layer_state) == _SYM;
 ...
 * Key Overrides
 */
const key_override_t override_rctl_w = ko_make_with_layers(MOD_BIT(KC_RCTL), KC_W, LCTL(KC_BSPC), (1 << _BASE | 1 << _NUM | 1 << _SYM));
const key_override_t override_rctl_h1 = ko_make_with_layers(MOD_BIT(KC_RCTL), KC_H, KC_BSPC, 1 << _BASE);
const key_override_t override_rctl_h2 = ko_make_with_layers(MOD_BIT(KC_RCTL), KC_6, KC_BSPC, 1 << _NUM);
...
bool process_record_keymap(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case VIM_G:
            if (record->event.pressed) {
                register_code16(S(KC_G));
            } else {
                unregister_code16(S(KC_G));
                if (is_num_word_on()) {
                    num_word_off();
                }
            }
